...
import { announcements as mock } from '@/lib/harar-data'
...
export const Route = createFileRoute('/news')({
  ...
  head: async () =>
    buildHeadAsync({
      ...
      description: 'Stay updated on events, notices, and stories from the Harari Regional Tourism Bureau.',
      canonicalPath: '/news',
    }),
  component: NewsPage,
})
...
function NewsPage() {
  ...
  return (
    <PublicLayout>
      <PageHero title="News & Announcements" subtitle="Stay updated on events, notices and stories from the Bureau." />
          ...
          {tabs.map((t) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium border transition-colors ${
                tab === t ? 'bg-brand text-white border-brand' : 'bg-white border-border hover:border-brand'
              }`}
            >
              {t}
              {t !== 'All' ? 's' : ''}
        ...
        {pinned && (
          <div className="mb-8">
            <AnnouncementCard
              ...
              excerpt={stripHtml(pinned.body ?? '').slice(0, 180) || 'Read more…'}
              pinned
            />
          </div>
        )}
