...
import { AdminLayout, AdminCard, Button, Toggle, Field, Input, Textarea } from '@/components/AdminLayout'
...
import { createAlbum, deleteAlbum, getAlbums, reorderAlbums, updateAlbum } from '@/lib/gallery-fns'
...
function GalleryAdmin() {
  ...
  async function onCreate() {
    ...
    setCreating(true)
    try {
      await createAlbum({ data: { title: newTitle.trim(), description: newDesc.trim() || undefined } })
  ...
  async function onDelete(id: string, title: string) {
    if (!window.confirm(`Delete album "${title}"? This will delete ALL items inside it.`)) return
    try {
        ...
        <AdminCard className="p-6 mb-6">
          <div className="grid md:grid-cols-2 gap-5">
            <Field label="Album Title">
              <Input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="Harar Jugol Walls" />
            </Field>
            <Field label="Description (optional)">
              <Textarea value={newDesc} onChange={(e) => setNewDesc(e.target.value)} rows={3} placeholder="Short description for visitors…" />
            </Field>
          </div>
          <div className="mt-4 flex gap-2 justify-end">
            ...
            <Button onClick={() => void onCreate()} disabled={creating || !newTitle.trim()}>
