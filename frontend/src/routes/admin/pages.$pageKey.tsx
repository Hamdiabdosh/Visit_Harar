...
import { getPage, publishPage, unpublishPage, upsertPageContent } from '@/lib/pages-fns'
...
export const Route = createFileRoute('/admin/pages/$pageKey')({
  loader: async ({ params }) => {
    const key = params.pageKey as any
...
function PageEditorShell() {
  ...
  const upsert = useMutation({
    mutationFn: (payload: { hero_image: string | null; content: unknown; publish?: boolean }) =>
      upsertPageContent({
        data: {
          page_key: key,
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin', 'page', key] }),
  ...
  return (
    <AdminLayout
      ...
      action={
        <Link to="/admin/pages" className="text-sm text-ink-muted inline-flex items-center gap-1 hover:text-brand">
      ...
      <AdminCard className="p-5 mb-6 flex items-center justify-between">
        <div className="text-sm text-ink-muted">
          ...
          {fresh.updated_by_name ? (
            <>
              {' '}
              by <span className="text-ink font-medium">{fresh.updated_by_name}</span>
            </>
          ) : null}
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => void upsert.mutate({ hero_image: fresh.hero_image, content: fresh.content })} disabled={upsert.isPending}>
            Save Draft
          </Button>
          <Button onClick={() => void publish.mutate()} disabled={publish.isPending}>
            Publish
          </Button>
          <Button variant="outline" onClick={() => void unpublish.mutate()} disabled={unpublish.isPending}>
            Unpublish
          </Button>
        </div>
      </AdminCard>
