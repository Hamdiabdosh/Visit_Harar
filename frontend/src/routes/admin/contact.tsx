...
import { contactInputSchema, type ContactInput } from '@/lib/validators/contact'
  ...
  return (
    ...
    >
      <AdminCard className="p-6 space-y-5 max-w-3xl">
        <Field label="Office Name"><Input {...form.register('office_name')} placeholder="Harari Regional Tourism Bureau" /></Field>
        <Field label="Address Line 1"><Input {...form.register('address_line1')} placeholder="Harar Jugol" /></Field>
        <Field label="Address Line 2"><Input {...form.register('address_line2')} placeholder="Harari Regional State" /></Field>
        <Field label="Country"><Input {...form.register('country')} placeholder="Ethiopia" /></Field>
        <div className="grid grid-cols-2 gap-3">
          <Field label="Phone (Primary)"><Input {...form.register('phone_primary')} placeholder="+251…" /></Field>
          <Field label="Phone (Secondary)"><Input {...form.register('phone_secondary')} placeholder="+251…" /></Field>
          <Field label="Email (General)"><Input {...form.register('email_general')} placeholder="info@visitharar.gov.et" /></Field>
          <Field label="Email (Bookings)"><Input {...form.register('email_bookings')} placeholder="bookings@visitharar.gov.et" /></Field>
        </div>
          ...
          <div className="space-y-2">
            {hours.map((h, i) => (
              <div key={i} className="grid grid-cols-[1fr_2fr_auto] gap-2 items-center">
                ...
                <button
                  type="button"
                  onClick={() => form.setValue('working_hours', hours.filter((_, j) => j !== i), { shouldDirty: true })}
            ...
            <Button
              type="button"
              variant="outline"
              onClick={() => form.setValue('working_hours', [...hours, { day: '', hours: '' }], { shouldDirty: true })}
            >
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Latitude"><Input type="number" step="any" {...form.register('map_lat', { valueAsNumber: true })} /></Field>
            <Field label="Longitude"><Input type="number" step="any" {...form.register('map_lng', { valueAsNumber: true })} /></Field>
          </div>
          ...
          <p className="text-[11px] text-ink-muted mt-2">📍 Pin will appear at these coordinates on the public site.</p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {["Facebook","Twitter","Instagram"].map((s) => (
              <div key={s} className="grid grid-cols-[100px_1fr] items-center gap-2">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-border">
          ...
          <div className="flex gap-2">
            <Button type="button" variant="outline" onClick={() => save.mutate({ ...form.getValues(), is_published: false })}>
              Save Draft
            </Button>
            <Button type="button" onClick={() => save.mutate({ ...form.getValues(), is_published: true })}>
              Save (Published)
            </Button>
          </div>
        </div>
