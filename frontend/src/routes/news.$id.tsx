  ...
  return (
    <PublicLayout>
      <div className="h-[40vh] bg-surface relative overflow-hidden">
        {item.cover_image ? (
          <img src={item.cover_image} alt="" className="absolute inset-0 w-full h-full object-cover" />
        ) : (
      ...
      <article className="max-w-3xl mx-auto px-5 lg:px-8 -mt-24 relative pb-20">
        <div className="bg-white rounded-lg border border-border shadow-lg p-10">
          <Link to="/news" className="text-ink-muted text-sm inline-flex items-center gap-1 hover:text-brand mb-4">
            <ArrowLeft className="w-4 h-4" /> All news
          </Link>
          <div className="flex items-center gap-2 text-xs mb-3">
            <span className="px-2 py-0.5 rounded-full bg-brand/10 text-brand font-semibold">{item.type}</span>
