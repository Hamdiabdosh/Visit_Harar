...
export const Route = createFileRoute("/culture")({
  ...
  head: async ({ loaderData }) => {
    ...
    const content =
      page && !isCultureContentEmpty(page.content)
        ? (page.content as { intro_text?: string })
        : CULTURE_DEFAULT_CONTENT;
...
function CulturePage() {
  ...
  const title = page?.title ?? "Culture & Festivals";
  const content =
    page && !isCultureContentEmpty(page.content)
  ...
  return (
    <PublicLayout>
      <PageHero
        title={title}
        subtitle="The living traditions of the Harari people."
