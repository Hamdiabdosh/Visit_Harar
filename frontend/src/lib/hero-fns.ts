...
export const getHero = createServerFn({ method: 'GET' }).handler(
  async (): Promise<HeroDto | null> => {
    try {
      ...
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to load hero')
    }
  },
)
...
export const upsertHero = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }: { data: HeroInput }): Promise<HeroDto> => {
    try {
      ...
      const [inserted] = await db.insert(heroContent).values(values).returning()
      return rowToHeroDto(inserted!, editor.name)
    } catch (err) {
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to save hero')
    }
  })
...
export const removeHeroBackground = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }: { data: { url: string } }) => {
    try {
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to delete image')
    }
  })
