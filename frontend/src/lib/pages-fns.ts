...
function rowToDto(row: typeof pages.$inferSelect, updatedByName?: string | null): PageDto {
  return {
    id: row.id,
...
const aboutContentSchema = z.object({
  ...
  quick_facts: z
    .array(z.object({ label: z.string().min(1).max(50), value: z.string().min(1).max(120) }))
    .optional(),
})
...
export const getPage = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data: key }): Promise<PageDto | null> => {
    try {
      const row = await db.query.pages.findFirst({ where: eq(pages.pageKey, key) })
      if (!row) return null
      let updatedByName: string | null = null
      if (row.updatedBy) {
        const [u] = await db.select({ name: user.name }).from(user).where(eq(user.id, row.updatedBy)).limit(1)
      ...
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to load page')
    }
  })
...
export const upsertPageContent = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<PageDto> => {
    try {
      ...
      const existing = await db.query.pages.findFirst({ where: eq(pages.pageKey, data.page_key) })
      if (!existing) throw createError('NOT_FOUND', 'Page seed missing — run db:seed')

      const [updated] = await db
        .update(pages)
        .set({
          heroImage: data.hero_image !== undefined ? (data.hero_image ?? null) : existing.heroImage,
          content: validated,
          updatedBy: editor.id,
          updatedAt: new Date(),
        })
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to save page')
    }
  })
...
export const publishPage = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data: key }): Promise<PageDto> => {
    try {
      const editor = await requireEditorSession()
      const existing = await db.query.pages.findFirst({ where: eq(pages.pageKey, key) })
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to publish')
    }
  })
...
export const unpublishPage = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data: key }): Promise<PageDto> => {
    try {
      ...
      const [updated] = await db
        .update(pages)
        .set({ isPublished: false, updatedBy: editor.id, updatedAt: new Date() })
        .where(eq(pages.pageKey, key))
        .returning()
      return rowToDto(updated!, editor.name)
    } catch (err) {
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to unpublish')
    }
  })
