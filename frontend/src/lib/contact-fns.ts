...
function rowToDto(row: typeof contactInfo.$inferSelect, updatedByName?: string | null): ContactDto {
  return {
    id: row.id,
...
function inputToRowValues(data: z.infer<typeof contactInputSchema>, userId: string) {
  return {
...
async function fetchContactDto(publishedOnly?: boolean): Promise<ContactDto | null> {
...
export const getContactInfo = createServerFn({ method: 'GET' }).handler(
  async (): Promise<ContactDto | null> => {
    try {
      ...
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to load contact')
    }
  },
)

export const getPublishedContactInfo = createServerFn({ method: 'GET' }).handler(
  async (): Promise<ContactDto | null> => {
    try {
...
export const upsertContactInfo = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<ContactDto> => {
    try {
      ...
      const [inserted] = await db.insert(contactInfo).values(values).returning()
      return rowToDto(inserted!, editor.name)
    } catch (err) {
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to save contact')
    }
  })
...
export const publishContact = createServerFn({ method: 'POST' }).handler(
  async (): Promise<ContactDto> => {
    try {
      ...
      const existing = await db.select().from(contactInfo).limit(1)
      const prev = existing[0]
      if (!prev) throw createError('NOT_FOUND', 'Contact info not configured yet')
