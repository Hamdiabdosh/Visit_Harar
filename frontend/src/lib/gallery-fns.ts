...
import { galleryAlbums, galleryItems } from '../../drizzle/schema/index'
import { auth } from '@/lib/auth.server'
import { createError, isAppError } from '@/lib/errors'
import { DB_SETUP_HINT, isDbUnavailableError } from '@/lib/db-errors'
import { deleteAsset, publicIdFromUrl, uploadAssetBuffer } from '@/lib/cloudinary.server'
...
function albumRowToDto(row: typeof galleryAlbums.$inferSelect, itemCount: number): GalleryAlbumDto {
  return {
    id: row.id,
    title: row.title,
...
export const getAlbums = createServerFn({ method: 'GET' }).handler(async (): Promise<GalleryAlbumDto[]> => {
  try {
    const rows = await db
      .select({
        album: galleryAlbums,
    ...
    throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to list albums')
  }
})

export const getAlbumById = createServerFn({ method: 'GET' })
  .inputValidator((id: unknown) => z.string().uuid().parse(id))
  .handler(async ({ data: id }): Promise<{ album: GalleryAlbumDto; items: GalleryItemDto[] } | null> => {
    try {
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to load album')
    }
  })
...
export const createAlbum = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<GalleryAlbumDto> => {
    try {
      ...
      const [{ maxOrder }] = await db.select({ maxOrder: max(galleryAlbums.sortOrder) }).from(galleryAlbums)
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to create album')
    }
  })
...
export const updateAlbum = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data: { id, data } }): Promise<GalleryAlbumDto> => {
    try {
      ...
      const existing = await db.query.galleryAlbums.findFirst({ where: eq(galleryAlbums.id, id) })
      ...
      const [updated] = await db
        ...
        .set({
          ...
          coverImage: data.cover_image !== undefined ? data.cover_image : existing.coverImage,
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to update album')
    }
  })
...
export const deleteAlbum = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data: id }): Promise<{ success: true }> => {
    try {
      await requireEditorSession()
      const items = await db.query.galleryItems.findMany({ where: eq(galleryItems.albumId, id) })
      ...
      await Promise.all(
        items.map(async (it) => {
          ...
          await deleteAsset(publicId, type === 'video' ? 'video' : 'image').catch(() => undefined)
        }),
      )
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to delete album')
    }
  })

export const reorderAlbums = createServerFn({ method: 'POST' })
  .inputValidator((ids: unknown) => z.array(z.string().uuid()).min(1).parse(ids))
  .handler(async ({ data: orderedIds }): Promise<{ success: true }> => {
    try {
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to reorder albums')
    }
  })
...
export const uploadMediaItem = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<GalleryItemDto> => {
    try {
      const editor = await requireEditorSession()
      const album = await db.query.galleryAlbums.findFirst({ where: eq(galleryAlbums.id, data.albumId) })
      if (!album) throw createError('NOT_FOUND', 'Album not found')

      const resourceType: 'image' | 'video' = data.mime.startsWith('video/') ? 'video' : 'image'
      ...
      if (!album.coverImage) {
        await db.update(galleryAlbums).set({ coverImage: uploaded.url, updatedAt: new Date() }).where(eq(galleryAlbums.id, data.albumId))
      }
...
export const updateMediaItem = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data: { id, data } }): Promise<GalleryItemDto> => {
    try {
      await requireEditorSession()
      const existing = await db.query.galleryItems.findFirst({ where: eq(galleryItems.id, id) })
      ...
      const nextAlt = data.alt_text !== undefined ? data.alt_text : existing.altText
      if (nextPublished && !nextAlt?.trim()) {
        throw createError('VALIDATION_ERROR', 'Alt text is required before publishing')
      }
      ...
      const [row] = await db
        ...
        .set({
          ...
          altText: data.alt_text !== undefined ? data.alt_text : existing.altText,
          isPublished: data.is_published ?? existing.isPublished,
        })
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to update item')
    }
  })
...
export const deleteMediaItem = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data: id }): Promise<{ success: true }> => {
    try {
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to delete item')
    }
  })
      ...
      await db.transaction(async (tx) => {
        for (let i = 0; i < ids.length; i++) {
          await tx
            .update(galleryItems)
            .set({ sortOrder: i })
            .where(and(eq(galleryItems.albumId, albumId), eq(galleryItems.id, ids[i]!)))
        }
      })
      return { success: true }
    } catch (err) {
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to reorder items')
    }
  })

export const setAlbumCover = createServerFn({ method: 'POST' })
  .inputValidator((raw: unknown) => z.object({ albumId: z.string().uuid(), itemId: z.string().uuid() }).parse(raw))
  .handler(async ({ data: { albumId, itemId } }): Promise<{ success: true }> => {
    try {
      await requireEditorSession()
      const item = await db.query.galleryItems.findFirst({ where: and(eq(galleryItems.id, itemId), eq(galleryItems.albumId, albumId)) })
      if (!item) throw createError('NOT_FOUND', 'Item not found')
      await db.update(galleryAlbums).set({ coverImage: item.url, updatedAt: new Date() }).where(eq(galleryAlbums.id, albumId))
      return { success: true }
    } catch (err) {
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to set cover')
    }
  })

export const bulkPublish = createServerFn({ method: 'POST' })
  .inputValidator((raw: unknown) => z.object({ ids: z.array(z.string().uuid()).min(1), published: z.boolean() }).parse(raw))
  .handler(async ({ data: { ids, published } }): Promise<{ success: true }> => {
    try {
      await requireEditorSession()
      if (published) {
        const rows = await db.select().from(galleryItems).where(inArray(galleryItems.id, ids))
        const missingAlt = rows.some((r) => !r.altText?.trim())
        if (missingAlt) {
          throw createError('VALIDATION_ERROR', 'Alt text is required before publishing')
        }
      }
      await db.update(galleryItems).set({ isPublished: published }).where(inArray(galleryItems.id, ids))
      return { success: true }
    } catch (err) {
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to bulk update')
    }
  })

// Public helpers
export const getPublishedAlbums = createServerFn({ method: 'GET' }).handler(async (): Promise<GalleryAlbumDto[]> => {
  try {
    const rows = await db
      .select({
        album: galleryAlbums,
    ...
    if (isAppError(err)) throw err
    throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to list')
  }
})

export const getPublishedAlbumItems = createServerFn({ method: 'GET' })
  .inputValidator((albumId: unknown) => z.string().uuid().parse(albumId))
  .handler(async ({ data: albumId }): Promise<{ album: GalleryAlbumDto; items: GalleryItemDto[] } | null> => {
    try {
      const album = await db.query.galleryAlbums.findFirst({
        where: and(eq(galleryAlbums.id, albumId), eq(galleryAlbums.isPublished, true)),
      })
      if (!album) return null
      const items = await db.query.galleryItems.findMany({
        where: and(eq(galleryItems.albumId, albumId), eq(galleryItems.isPublished, true)),
        orderBy: [asc(galleryItems.sortOrder), desc(galleryItems.createdAt)],
      })
      const [{ itemCount }] = await db
        .select({ itemCount: count(galleryItems.id) })
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to load')
    }
  })
...
export const getLatestGalleryItems = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data: n }): Promise<GalleryItemDto[]> => {
    try {
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to load latest items')
    }
  })
