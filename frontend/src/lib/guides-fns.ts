...
function rowToDto(row: typeof guides.$inferSelect, extras?: Partial<GuideDto>): GuideDto {
  return {
    id: row.id,
    name: row.name,
    slug: row.slug,
...
async function ensureUniqueGuideSlug(slug: string, excludeId?: string): Promise<string> {
  let candidate = slug
  let n = 2
  while (true) {
...
export const getGuides = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data: filters }): Promise<GuideDto[]> => {
    try {
      ...
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to list guides')
    }
  })
...
export const getGuideBySlug = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data: slug }): Promise<GuideDto | null> => {
    try {
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to load guide')
    }
  })
...
export const createGuide = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<GuideDto> => {
    try {
      ...
      const [{ maxOrder }] = await db.select({ maxOrder: max(guides.sortOrder) }).from(guides)
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to create guide')
    }
  })
...
export const updateGuide = createServerFn({ method: 'POST' })
  .inputValidator((raw: unknown) => {
    const schema = z.object({
      id: z.string().uuid(),
      data: guideInputSchema.partial().extend({ name: z.string().min(1).max(200).optional() }),
    })
    ...
    try {
      ...
      const [updated] = await db
        ...
        .set({
          ...
          experienceYears:
            data.experience_years !== undefined ? data.experience_years ?? null : existing.experienceYears,
          licenseNumber:
            data.license_number !== undefined ? data.license_number ?? null : existing.licenseNumber,
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to update guide')
    }
  })
  ...
  .handler(async ({ data: id }): Promise<{ success: true }> => {
    try {
      ...
      if (Number(bookingCount ?? 0) > 0) {
        throw createError('CONFLICT', 'Cannot delete a guide with existing bookings')
      }
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to delete guide')
    }
  })
...
export const reorderGuides = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data: orderedIds }): Promise<{ success: true }> => {
    try {
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to reorder')
    }
  })
...
export const toggleGuidePublished = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data: id }): Promise<{ is_published: boolean }> => {
    try {
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to toggle')
    }
  })
...
export const uploadGuidePhoto = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<{ url: string; publicId: string }> => {
    try {
      ...
      return await uploadImageBuffer(buffer, data.filename, 'visit-harar/guides')
    } catch (err) {
      if (isAppError(err)) throw err
