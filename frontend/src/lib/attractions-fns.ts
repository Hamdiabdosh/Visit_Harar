...
async function mapRowWithName(row: typeof attractions.$inferSelect): Promise<AttractionDto> {
...
export const getAttractions = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data: filters }): Promise<AttractionDto[]> => {
    try {
      ...
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to list attractions')
    }
  })
  ...
  .handler(async ({ data: slug }): Promise<AttractionDto | null> => {
    try {
      const row = await db.query.attractions.findFirst({
        where: and(eq(attractions.slug, slug), eq(attractions.isPublished, true)),
      })
      if (!row) return null
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to load attraction')
    }
  })
...
export const createAttraction = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }: { data: z.infer<typeof attractionInputSchema> }): Promise<AttractionDto> => {
    try {
      ...
      const [inserted] = await db
        .insert(attractions)
        .values({
          ...inputToRowValues({ ...data, sort_order: sortOrder }, editor.id, slug),
          createdBy: editor.id,
        })
        .returning()
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to create')
    }
  })
  ...
  .handler(async ({ data: { id, data } }): Promise<AttractionDto> => {
    try {
      ...
      const merged = {
        ...
        slug,
        shortDesc: data.short_desc !== undefined ? data.short_desc ?? null : existing.shortDesc,
        fullDesc: data.full_desc !== undefined ? data.full_desc ?? null : existing.fullDesc,
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to update')
    }
  })
...
export const deleteAttraction = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data: id }): Promise<{ success: true }> => {
    try {
      ...
      if (isAppError(err)) throw err
      throw createError('INTERNAL', err instanceof Error ? err.message : 'Failed to delete')
    }
  })
...
export const toggleAttractionPublished = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data: id }): Promise<{ is_published: boolean }> => {
    try {
      const editor = await requireEditorSession()
      const row = await db.query.attractions.findFirst({ where: eq(attractions.id, id) })
  ...
  .handler(
    async ({
      ...
      try {
        ...
        return await uploadImageBuffer(buffer, data.filename, 'visit-harar/attractions')
      } catch (err) {
        if (isAppError(err)) throw err
