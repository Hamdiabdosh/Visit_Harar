    ...
    <footer className="bg-brand-dark text-white/80 pt-16 pb-6 mt-20">
      <div className="max-w-7xl mx-auto px-5 lg:px-8 grid grid-cols-2 lg:grid-cols-6 gap-10">
        <div className="col-span-2">
          <div className="flex items-center gap-3 mb-4">
            <span className="w-10 h-10 rounded-full bg-gold text-brand-dark grid place-items-center font-serif text-lg font-bold">
              H
            </span>
            <span className="font-serif text-white text-xl font-bold">Visit Harar</span>
          </div>
          <p className="text-sm leading-relaxed max-w-sm">
            Official tourism website of the Harari Regional Tourism Bureau — Africa&apos;s fourth holiest Islamic city
            and a UNESCO World Heritage Site.
          </p>

          <div className="flex items-center gap-3 mt-5">
            <SocialLink href={contact?.facebook_url} label="Facebook" icon={<Facebook className="w-4 h-4" />} />
            <SocialLink href={contact?.twitter_url} label="Twitter" icon={<Twitter className="w-4 h-4" />} />
            <SocialLink href={contact?.instagram_url} label="Instagram" icon={<Instagram className="w-4 h-4" />} />
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-5 lg:px-8 mt-10 pt-6 border-t border-white/10 text-xs flex flex-col md:flex-row justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <span className="text-white/60">© {year} Harari Regional Tourism Bureau</span>
...
function SocialLink({ href, label, icon }: { href: string | null | undefined; label: string; icon: React.ReactNode }) {
  if (!href) return null
  return (
    <a
      href={href}
