...
import { uploadImageBuffer } from "../src/lib/cloudinary.server";
...
export async function seedCloudinaryImages() {
  if (
    !process.env.CLOUDINARY_CLOUD_NAME ||
    !process.env.CLOUDINARY_API_KEY ||
    !process.env.CLOUDINARY_API_SECRET
  ) {
    console.log("⊘ Cloudinary not configured — skipping image seed");
    return;
  }
  ...
  if (heroUpload) {
    ...
    console.log("✓ Hero background uploaded to Cloudinary");
  }
  ...
  console.log("✓ Attraction images uploaded to Cloudinary");
  ...
  console.log("✓ Guide photos uploaded to Cloudinary");
  ...
  if (albums.length > 0) {
    ...
    console.log(
      `✓ Gallery items uploaded to Cloudinary (${totalItems} items across ${albums.length} albums)`,
    );
  }
}
