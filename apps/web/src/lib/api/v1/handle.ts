...
export async function handleV1Request(
  ...
  try {
    ...
    if (method === "GET" && parts[0] === "bookings" && parts[1] === "enabled") {
      const data = await getBookingEnabled();
      return jsonOk({ enabled: data }, { cache: CACHE_PUBLIC_SHORT });
    }

    if (method === "GET" && parts[0] === "push" && parts[1] === "enabled") {
      const data = await getPushEnabled();
      return jsonOk(data, { cache: CACHE_PUBLIC_SHORT });
    }

    if (method === "POST" && parts[0] === "push" && parts.length === 2) {
      const ip = clientIp(request);
      if (parts[1] === "register") {
        checkRateLimit(`push-register:${ip}`, 30, 60_000);
        const body = await readJsonBody(request);
        const result = await registerPushSubscription(body);
        return jsonOk(result);
      }
      if (parts[1] === "unregister") {
        checkRateLimit(`push-unregister:${ip}`, 30, 60_000);
        const body = await readJsonBody(request);
        const result = await unregisterPushSubscription(body);
        return jsonOk(result);
      }
    }
    ...
    if (method === "POST" && parts[0] === "events" && parts[1] === "registrations") {
      const ip = clientIp(request);
      if (parts.length === 2) {
        checkRateLimit(`event-registrations:${ip}`, 10, 60_000);
        const body = await readJsonBody(request);
        const result = await createEventRegistration({ data: body });
        return jsonOk(result);
      }
      if (parts.length === 3 && parts[2] === "status") {
        checkRateLimit(`event-registration-status:${ip}`, 20, 60_000);
        const body = await readJsonBody(request);
        const result = await getEventRegistrationByRef({ data: body });
        if (!result) {
          return jsonError("NOT_FOUND", "Registration not found", 404);
        }
        return jsonOk(result);
      }
    }
    ...
    if (
      ...
      const result = await getEventTicketByToken({ data: parts[2]! });
      if (!result) return jsonError("NOT_FOUND", "Ticket not found", 404);
      return jsonOk(result, { cache: CACHE_PUBLIC_SHORT });
    }
