...
export function GuideCard({
  ...
  const src = photo ? pickListImageUrl(photo) : null;
  return (
    <Link
      to="/guides/$slug"
      params={{ slug }}
    ...
    >
      ...
      <div className="flex flex-wrap gap-1.5 mt-4">
        {languages.slice(0, 4).map((l) => (
          <span
            key={l}
            className="text-[10px] px-2 py-0.5 rounded bg-surface border border-border"
          >
            {l}
          </span>
        ))}
      ...
      <div className="flex flex-wrap gap-1.5 mt-2">
        {specialties.slice(0, 4).map((s) => (
          <span
            key={s}
            className="text-[10px] px-2 py-0.5 rounded bg-gold/15 text-amber-900"
          >
            {s}
          </span>
        ))}
      ...
      <div className="flex items-center justify-between mt-5">
        <span className="text-xs text-ink-muted inline-flex items-center gap-2">
          <span
            className={`w-2 h-2 rounded-full ${is_available ? "bg-emerald-500" : "bg-gray-400"}`}
          />
